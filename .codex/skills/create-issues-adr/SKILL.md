...
6. 사용자에게 feature별 issue map과 ADR 후보를 제시하고 확정받는다.
...
- 로컬 이슈 파일은 `docs/features/{feature}/issues/{feature}-{nnn}-{slug}.md` 형식으로 작성한다.
- 로컬 이슈 ID는 `{feature}-{nnn}`이고, 파일명 slug는 내용을 설명하는 소문자 hyphen-case 영문으로 작성한다.
...
- 스크립트가 감지하지 못하는 새 파일명 규칙이나 frontmatter 누락이 의심되면 파일명과 본문 제목을 직접 확인한다.

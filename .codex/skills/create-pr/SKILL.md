---
name: create-pr
description: 현재 브랜치의 변경 사항을 파악하고 전체 테스트를 통과시킨 뒤, 한국어 PR 초안을 작성해 사용자 승인 후 origin에 push하고 GitHub Pull Request를 생성해야 할 때 사용한다. git diff/status/log 기반 변경 요약, 테스트 검증, # 변경사항/# 검증/# 참고 형식의 PR 본문, 승인 게이트, gh CLI 또는 GitHub MCP/app connector를 통한 PR 생성, 실패 시 중단 보고에 사용한다.
---
...
5. 사용자에게 PR 제목과 본문 초안을 보여주고 “이 내용으로 push 및 PR 생성을 진행할지” 확인한다.
6. 사용자가 승인하면 원격 브랜치에 push한다.
...
8. 완료 시 PR URL, 사용한 push/PR 생성 경로, 실행한 검증 명령을 간단히 보고한다.

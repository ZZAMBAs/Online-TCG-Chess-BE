---
name: create-prd
description: docs/spec/spec-fixed.md를 원천 명세로 읽고 현재 요구사항에 맞는 feature 단위를 유동적으로 도출한 뒤 사용자 승인 게이트를 거쳐 docs/prd.md, docs/trd.md, docs/traceability.md, docs/milestones.md, docs/websocket-spec.md와 docs/features/{feature}/prd.md, trd.md 초안을 한국어로 생성해야 할 때 사용한다. PRD/TRD 분리, feature별 사용자 시나리오와 엣지 케이스 재배치, 권한/상태별 제한, 실패 시 상태 불변성, 동시성/멱등성, 오류 코드, 테스트 관점, 원천 섹션 추적, WebSocket 공통 계약 초안, 마일스톤 초안 작성에 사용하며 spec-fixed.md 수정, 요구사항 인터뷰, 스펙 리뷰, issue 분리, ADR 작성은 하지 않는다.
---
...
- 사용자가 명시적으로 일괄 진행을 요청하지 않는 한, 공통 문서와 feature별 PRD/TRD는 문서별 사용자 승인 게이트를 통과한 뒤 다음 문서로 진행한다.
...
6. 파일 작성 전에 feature map, 공통 문서 작성 순서, feature 문서 작성 순서를 세션에서 제시하고 사용자 승인을 받는다.
7. 아래 문서 구조와 작성 기준을 따른다.
8. 다음 공통 문서를 순서대로 하나씩 생성하거나 갱신한다. 각 문서 초안 작성 후 사용자에게 확정 여부를 묻고, 확정되기 전에는 다음 문서로 진행하지 않는다.
   - `docs/prd.md`
   - `docs/trd.md`
   - `docs/traceability.md`
   - `docs/milestones.md`
   - `docs/websocket-spec.md`
9. 각 feature마다 다음 문서를 순서대로 하나씩 생성하거나 갱신한다. 각 문서 초안 작성 후 사용자에게 확정 여부를 묻고, 확정되기 전에는 다음 문서로 진행하지 않는다.
   - `docs/features/{feature}/prd.md`
   - `docs/features/{feature}/trd.md`
10. 사용자가 확정하지 않은 문서는 수정 요청을 반영해 다시 제시하고, 확정 전에는 후속 문서의 기준으로 삼지 않는다.
...
- 확정된 문서의 결정사항은 후속 문서에서 기준으로 사용한다.
...
`docs/websocket-spec.md`는 feature TRD에 중복하지 않을 STOMP/WebSocket 공통 계약 기준 문서다. 원천 명세에 확정된 내용과 문서화 가정 또는 계약 후보를 구분해 작성한다.
...
destination, payload, 이벤트 필드는 원천 명세에 없으면 임의 확정하지 않고 "계약 후보" 또는 "미확정 사항"으로 표시한다.
...
- 공통 문서와 feature별 PRD/TRD가 문서별 사용자 승인 게이트를 통과한 뒤 다음 문서로 진행되었다.
...
- `docs/websocket-spec.md`가 필수 섹션을 포함하고, 원천 명세에 없는 실시간 계약을 확정값처럼 쓰지 않는다.

---
name: tdd-red
description: 로컬 feature 이슈 문서에 이미 정의된 사용자 시나리오, Given-When-Then AC, 테스트 시나리오를 Java/Spring 테스트 코드로 옮겨 TDD RED 단계를 수행해야 할 때 사용한다. 인자로 auth-1, auth-001, auth-issues-1, xxx-yyy-1처럼 마지막 토큰이 숫자인 이슈 식별자를 받아 docs/features/{feature}/issues의 이슈와 선행 이슈 상태를 확인하고, src/test/java 테스트와 현재 이슈 문서의 TDD RED 결과만 생성·수정하며 새 사용자 시나리오/새 AC/새 정책값, src/main, production skeleton, build.gradle, CI, 정적 분석 도구 설정, production resource는 만들지 않는다.
---
...
로컬 이슈에 이미 정의된 사용자 시나리오, AC, 테스트 시나리오를 실패 가능한 RED 테스트 코드로 옮긴다. RED 단계에서는 테스트 코드와 현재 이슈 문서의 RED 결과 기록만 작성하고, 새 사용자 시나리오/새 AC/새 정책값, production 구현, production skeleton, 하네스 도입은 하지 않는다.
...
4. FE/API 계약 영향을 확인하기 위한 `docs/spec/spec-fixed.md`, `docs/websocket-spec.md`, 이슈 frontmatter 또는 본문에 적힌 `docs/contracts/{topic}.md`, CORS/OpenAPI/JSON Schema/fixture 흔적
5. 테스트 대상 패키지와 기존 테스트 패턴을 파악하기 위한 `src/main/java`와 `src/test/java`
6. `TDD RED 결과`를 기록하기 위한 `references/red-result-contract.md`

PRD/TRD가 없거나 이슈 내용만으로 테스트 대상, 패키지, 테스트 파일명, 테스트 시나리오를 결정할 수 없으면 사용자에게 확인을 요청한다.
...
- 이슈 문서에 없는 새 사용자 시나리오, 새 AC, 새 정책값, 새 FE/BE 계약, 새 테스트 요구사항 생성

이미 하네스가 존재하는 경우에만 RED 테스트에서 그 하네스를 사용할 수 있다. 하네스가 없으면 `blocked`로 보고하고 `실패 또는 차단 요약`에 하네스 부재를 기록한다.

## 테스트 작성 규칙

- 이슈의 `관련 사용자 시나리오`, `Acceptance Criteria`, `테스트 시나리오`, `테스트`, `구현 메모`를 기준으로 테스트 메서드를 작성한다.
- `[정상]`, `[경계]`, `[예외]` AC와 이슈에 정의된 테스트 시나리오는 각각 독립 테스트 메서드 후보로 변환한다.
- RED는 테스트 시나리오를 새로 도출하지 않는다. AC 또는 테스트 시나리오가 테스트 코드로 옮기기에 부족하면 추측하지 말고 `blocked`로 기록한다.
- REST 또는 STOMP가 사용자나 FE에 보이는 결과이면 단위 테스트만 만들지 말고 API/계약 테스트 후보를 우선 검토한다.
- 이슈가 `docs/contracts/{topic}.md`를 참조하면 해당 계약의 `implementation_status: ready` 여부와 BE가 지켜야 할 결정을 확인하고, 관련 AC 테스트에 반영한다. 필요한 계약 문서가 없거나 ready가 아니면 테스트를 추측하지 말고 `blocked`로 기록한다.
- OpenAPI, JSON Schema, fixture, contract-test 하네스가 필요하지만 없으면 테스트 도입을 추측하지 말고 `blocked`로 기록한다.
...
모든 테스트를 작성한 뒤 가능한 범위에서 전체 테스트를 실행한다. 보고할 때는 기존 테스트 실패, 테스트 코드 오류, 의도된 RED 실패, `already-green`, `compile-blocked`, `blocked`를 구분한다.

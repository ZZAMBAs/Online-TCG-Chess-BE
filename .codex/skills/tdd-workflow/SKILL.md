...
8. 커밋이 필요한 경우 `.codex/skills/git-commit/SKILL.md`
...
각 단계가 정상적으로 다음 단계로 넘어갈 수 있는 상태를 만들고, 작업트리에 해당 단계의 일관된 변경이 남아 있으면 `$git-commit`을 사용해 커밋할 수 있다. 커밋 여부 판단과 실행은 아래 규칙을 따른다.

- 커밋이 필요하면 먼저 `.codex/skills/git-commit/SKILL.md`를 읽고 그 계약에 따라 stage, 메시지 작성, hook 실패 처리를 위임한다.
- RED, GREEN, BLUE, security-review 보완, AC 검증 로그 갱신처럼 의미가 다른 작업 단위는 가능한 한 별도 커밋으로 분리한다.
- 단계 결과가 차단, 실패, 사용자 게이트이면 그 상태를 해결하기 위한 커밋을 자동 생성하지 않는다. 단, 사용자가 판단할 수 있도록 최신 `tdd-workflow.md` 기록만 필요한 경우 문서 변경을 남기고 보고한다.
- `$git-commit`이 hook 실패나 커밋 범위 불명확으로 중단하면 워크플로우도 다음 단계로 진행하지 않고 `workflow-blocked` 또는 `workflow-user-gated`로 멈춘다.
- 커밋 후에는 `git status --short --branch`와 필요한 diff/log를 다시 확인해 다음 단계가 깨끗한 기준에서 시작되는지 확인한다.

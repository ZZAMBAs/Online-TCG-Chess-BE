...
| module-boundaries | 2026-07-14 | cardcollection 공개 port로 gameplay가 활성 버전 조회 | false | 카드 catalog 책임 경계 확정 |
...
| data-cache-messaging-storage | 2026-07-14 | docs/cards 작성 원천, RDB 불변 버전·활성 버전 실행 원천 | false | 기존 미확정 카드 저장 방식 확정 |
...
| cicd | 2026-07-14 | 카드 catalog·effect 완결성 required check 조건 확정 | false | 새 배포 검증 경계 확정 |
| harness-guardrails | 2026-07-14 | 카드 catalog·effect·RDB 동기화 gate 추가 | false | 새 검증 gate 분류 |
...
- source_fingerprint: spec `6ac09a695776`, gameplay `9933c7a4097e`, card collection `e6e6d92fc333`, match history `3518bb02d796`
...
- decision_summary: `docs/cards` 작성·배포 원천, RDB의 모든 배포 불변 버전과 활성 버전 실행·조회, `GameEvent` 당시 결과 재현
- skip_conditions: 카드 정의 원천, 불변 버전, 활성 전환, 경기 pinning과 관련 의존성이 변하지 않음
- re_review_triggers: RDB 직접 authoring, 운영 UI 카드 편집, 외부 catalog/object storage, 다중 app version 동시 배포, historical replay 재계산 요구
- latest_result: ARCH-ADR-002와 fixed-20260714에 확정. 상세 persistence는 후속 TRD/TDD로 이관
...
- source_fingerprint: harness `859f9a3d0e1e`, cicd `b644c8ec30de`
...
- decision_summary: 기존 Hard/Conditional 분류 유지, 카드 catalog와 effect 완결성은 구현 즉시 Hard, RDB 동기화는 persistence 구현 시 Testcontainers 검증
- skip_conditions: catalog validator, effect registry, fixed contract와 persistence 구현이 아직 없고 관련 CI 설정이 변하지 않음
- re_review_triggers: catalog validator/effect registry/persistence 구현, FE/BE 카드 계약 fixed, Testcontainers 또는 CI workflow 변경
...
- source_fingerprint: server architecture `bb37db552c1f`
...
- decision_summary: `cardcollection`이 catalog 동기화·활성 버전 공개 port를 소유하고 `gameplay`는 port로 조회해 경기 버전을 고정
- skip_conditions: catalog 소유권과 module API가 변하지 않고 직접 repository 참조가 없음
- re_review_triggers: 카드 효과 처리 소유권 변경, module API 변경, cardcollection/gameplay 저장소 직접 참조, 별도 서비스 분리
...
- source_fingerprint: cicd `b644c8ec30de`, harness `859f9a3d0e1e`
...
- decision_summary: 활성 버전 목록·catalog·effect·contract 완결성은 구현 시 required check, RDB 충돌은 기동 차단
- skip_conditions: catalog 관련 구현과 배포 workflow가 아직 없고 기존 CI가 변하지 않음
- re_review_triggers: catalog task, CD, application startup sync, migration/rollback workflow 구현

...
- 버전 관리되는 `docs/card-packs` JSON과 별도 활성 버전 manifest를 카드팩 정의의 작성·배포 원천으로 사용한다.
- RDB는 검증·배포된 모든 카드팩 식별자·버전과 현재 활성 팩 버전의 실행·조회 원천이다. 같은 식별자·버전의 다른 내용은 기동을 실패시키고 누락된 새 버전만 추가한다.
- 활성 팩 버전은 RDB 직접 수정이 아니라 manifest를 변경한 새 배포에서만 전환한다.
- `cardcollection` 모듈이 카드와 카드팩 catalog 동기화·활성 조회 application port를 함께 소유한다.

## 문서화 가정

- 카드 catalog는 빌드 산출물에 포함되고 애플리케이션 기동 시 검증·동기화된다.
- 활성 버전 목록은 카드별 신규 경기에 사용할 버전을 하나만 가리킨다.
- 카드팩 활성 manifest는 팩 식별자별 실행에 사용할 버전을 하나만 가리킨다.
- 카드와 카드팩 catalog는 같은 배포 산출물과 검증 경계를 사용하되 서로 다른 version namespace와 schema를 유지한다.
...
- `create-trd`: 카드·카드팩 catalog resource, 동기화 경계, persistence와 effect registry 상세 설계

...
- RDB에는 검증·배포된 모든 카드·카드팩 식별자·버전과 각각의 현재 활성 버전을 보존한다.
- 카드·카드팩 version 동기화는 기존 동일 버전을 갱신하지 않고 새 버전 추가와 활성 버전 선택만 허용한다.
- 활성 버전 변경은 버전 관리되는 각 활성 manifest를 포함한 애플리케이션 배포를 통해서만 수행하며 운영 중 RDB 직접 변경을 허용하지 않는다.
...
- 카드·카드팩 정의 JSON과 각 활성 버전 목록은 애플리케이션 산출물에 포함하며 외부 object storage를 요구하지 않는다.

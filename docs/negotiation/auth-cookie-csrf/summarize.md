# Auth Cookie CSRF Contract Summary

## 상태

- status: fixed
- source_artifacts: `docs/negotiation/auth-cookie-csrf/20260713-141030.md`, `docs/negotiation/auth-cookie-csrf/20260713-141403.md`, `docs/negotiation/auth-cookie-csrf/20260713-141641.md`, `docs/negotiation/auth-cookie-csrf/20260713-142101.md`, `docs/negotiation/common-error-contract/summarize.md`
- supersedes: 없음
...
- 사용자 결정 반영: 2026-07-13 사용자가 중재자 추천안인 현재 browser session 단위 로그아웃을 승인했다.
- 일반 로그아웃은 현재 session과 연결된 CSRF·STOMP만 종료하며, 모든 기기 로그아웃은 별도 보안 기능으로 분리한다.
- 사용자 판단이 필요 없는 기술 항목은 FE/BE 검토에서 이견 없이 합의되었다.
- 후속 `common-error-contract`의 FE/BE 합의로 기존 code 의미를 유지한 채 `schemaVersion`과 transport별 닫힌 schema를 반영했다.

## 참조 메모 반영 대상

- BE: 전체 협상 session이 `completed`가 된 뒤 `$sync-be-contracts`가 `docs/contracts/auth-cookie-csrf.md`에 구현 참조 계약을 투영한다.
- FE: 전체 협상 session이 `completed`가 된 뒤 FE의 계약 동기화 워크플로가 canonical BE repo의 fixed 요약을 구현 참조 문서로 투영한다.

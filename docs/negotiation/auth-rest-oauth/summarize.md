# Auth REST OAuth Contract Summary

## 상태

- status: fixed
- source_artifacts: `docs/negotiation/auth-rest-oauth/20260713-154427.md`, `docs/negotiation/auth-rest-oauth/20260713-154534.md`, `docs/negotiation/auth-rest-oauth/20260713-154855.md`, `docs/negotiation/auth-rest-oauth/20260713-161627.md`, `docs/negotiation/auth-rest-oauth/20260713-161849.md`
- supersedes: 없음
...
- 기술 계약으로 확정 가능: 다른 browser email confirm 비로그인, token fragment 단회 POST, reset 완전 enumeration-safe `202`, OAuth server-session result와 fixed redirect, no linking.
- 사용자 결정 반영: 2026-07-13 withdrawal 전 local current password 또는 동일 OAuth provider fresh 재인가를 필수화했다.
- 사용자 결정 반영: local reactivation은 기존 password와 5분 email token 모두, OAuth reactivation은 동일 provider 재인가만 요구하며 provider 이용 불가 복구는 MVP에서 제외한다.
- FE/BE 2차 검토에서 승인된 proof matrix와 endpoint lifecycle이 session·CSRF·원자성·정보 은닉 조건에 부합함을 확인해 fixed로 확정했다.

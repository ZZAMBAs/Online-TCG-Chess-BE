# Common Error Contract Summary

## 상태

- status: fixed
- source_artifacts: `docs/negotiation/common-error-contract/20260713-153103.md`, `docs/negotiation/common-error-contract/20260713-153514.md`, `docs/negotiation/common-error-contract/20260713-153647.md`, `docs/negotiation/common-error-contract/20260713-153757.md`
- supersedes: 앞선 fixed topic에 흩어진 오류 전달 규칙을 공통 schema로 정합화한다.
...
- 사용자 판단이 필요 없는 FE/BE 기술 계약으로 이견 없이 확정했다.
- rate limit 임계값, 재시도 횟수와 제품 문구는 상위 근거 없이 새로 만들지 않았다.
- 기존 fixed auth의 status와 세부 code 의미는 유지하고 envelope version과 schema 이름만 정합화한다.

## 참조 메모 반영 대상

- BE: 전체 협상 session이 `completed`가 된 뒤 `$sync-be-contracts`가 `docs/contracts/common-error-contract.md`에 구현 참조 계약을 투영한다.

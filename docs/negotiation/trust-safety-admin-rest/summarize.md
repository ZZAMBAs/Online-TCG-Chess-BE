# Trust Safety Admin REST Contract Summary

## 상태

- status: fixed
- source_artifacts: `docs/negotiation/trust-safety-admin-rest/20260714-171052.md`, `docs/negotiation/trust-safety-admin-rest/20260714-171126.md`, `docs/negotiation/trust-safety-admin-rest/20260714-171237.md`, `docs/negotiation/trust-safety-admin-rest/20260714-171342.md`, `docs/negotiation/trust-safety-admin-rest/20260714-171712.md`, `docs/negotiation/trust-safety-admin-rest/20260714-171817.md`, `docs/negotiation/trust-safety-admin-rest/20260714-171926.md`, `docs/negotiation/trust-safety-admin-rest/20260714-172023.md`, `docs/negotiation/trust-safety-admin-rest/20260714-172240.md`, `docs/negotiation/trust-safety-admin-rest/20260714-172700.md`
- supersedes: 기존 fixed community/game projection에 action-scoped `subjectRef`와 report/block capability를 후속 확장한다.
...
- 사용자 결정: individual report + target-grouped case, OPEN/RESOLVED, ACTION_TAKEN/NO_ACTION, no IN_REVIEW/reopen.
- 사용자 결정: closed reason, optional 500 detail, post lifetime one report, chat/profile one-open report, self-report 금지, context-scoped subjectRef.
- 사용자 결정: chat/post/profile evidence snapshot과 resolved+30일 sensitive evidence 삭제.
- 사용자 결정: immediate sanction, preset/custom 1h~365d, one active per type, extension, self 금지, public/revoke reason 500.
- 사용자 결정: forbidden word 1~50, case-insensitive duplicate, change reason 1~200, actual PATCH와 non-retroactive masking.
- 사용자 결정: ACTION_TAKEN qualifying linked action 필수, exact replay 외 terminal command conflict, no reopen/no implicit no-op.
- 사용자 결정: communication suspension은 block/profile report 허용, chat/post report 거부; account suspension은 모두 거부.
- 사용자 결정: admin REST/UI에서 NORMAL/IMPORTANT announcement를 생성하고 optional/required inbox policy를 적용한다.
- 사용자 결정 2026-07-15: 공지 eligible population은 모든 non-WITHDRAWN account이며 미인증·계정정지·관리자도 포함한다.
- `contract-schema-closure`가 exact request/response/error union, capability merge와 canonical admin card `$ref`를 보완했다.
- FE/BE 최종 확인: fixed 가능, 추가 사용자 결정과 architecture 변경 없음.

## 참조 메모 반영 대상

- 전체 negotiation session이 completed된 뒤에만 BE/FE 구현 계약으로 동기화한다.
